...
def run_reference_monitor_base_test(ref_monitor):
    print("Running reference monitor against vt2182_attackcase2.r2py")
    call(["cd", reference_monitors_path])
    call(["rr2", ref_monitor, "vt2182_attackcase2.r2py"])
    call(["cd", ".."])
...
if os.path.exists(json_file_path) and os.path.getsize(json_file_path) > 0:
    ...
    if netid in data:
        ...
        if not ref_monitor_1:
            print("No submission for reference monitor 2.1 assignment")
        else:
            open_in_vscode(ref_monitor_1_path)

        # Run the reference monitor against base attack case
        run_reference_monitor_base_test(ref_monitor_1)

        if not ref_monitor_3:
            print("No submission for reference monitor 2.3 assignment")
        # else:
        #     open_in_vscode(ref_monitor_3_path)

        if not writeup:
            print("No submission for writeup")
        else:
        ...
        print("No data found for the provided netid.")
else:
    # If the JSON file is empty or doesn't exist, populate it
    ref_monitors_1 = find_files(reference_monitors_1_path , '.r2py')
    ref_monitors_3 = find_files(reference_monitors_3_path , '.r2py')
